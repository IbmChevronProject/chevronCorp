...
{
    public static class getPLmails
    {
        [FunctionName("getPLmails")]
...

            {
                ...
                try
                {
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        // Defining InterviewId to get PLmail
                        var InterviewId = req.Query["InterviewId"];

                        // Prepare the SQL Query
                        var query = $"select Username from Userdetails a inner join ServiceLine_PL b on a.userid=b.PanelListIds where InterviewId='{InterviewId}'";
